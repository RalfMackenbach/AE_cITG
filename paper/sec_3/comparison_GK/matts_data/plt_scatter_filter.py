...
kpar_4 = np.load('connection_length_estimate/20241103-01-assembleFluxTubeTensor_multiNfp_finiteBeta_randomAspect_nz96_51200tubes_kpar.npy')
# get the dictionary
...
kpar_all = np.concatenate((kpar_1,kpar_2,kpar_3,kpar_4)).flatten()
...
# construct a mask where mask is True if k_par < k_filter
k_filter = 0.0
mask = kpar_all <= k_filter
# convert False to NaN, true to 1
mask = mask.astype(float)
mask[mask==0] = np.nan

# apply the mask to the data by multiplying
AE_all = AE_all*mask
Q_all = Q_all*mask
...
loga = 5.949503985660509
print('loga: ',loga)
# plot the fit
x_fit = np.linspace(1e-3,1e0,100)
y_fit = np.exp(fit_func(np.log(x_fit),loga))
# add to all plots
for key in ax.keys():
...
# save the figure
plt.savefig('scatter_AE_Q_filter.png',dpi=1000)
