...
def calculate_AE_strong(w_alpha, w_psi, w_n, w_T,idx):
    dict = ae.calculate_AE_strong(w_alpha[idx], w_psi[idx], w_n[idx], w_T[idx])
    return dict
...
if __name__ == '__main__':
    ...
    # save results
    np.save('plots/data/AE_strong.npy',AE)
    np.save('plots/data/k_alpha_strong.npy',k_alpha)
    np.save('plots/data/k_psi_strong.npy',k_psi)
    ...
    if np.isnan(k_psi_max):
        k_psi_max = 0
    if np.isnan(k_psi_min):
        k_psi_min = 0
    k_psi_lvls  = np.linspace(k_psi_min,k_psi_max,lvls_res)
    ...
    plt.savefig('plots/AE_strong.png',dpi=1000)
    plt.show()