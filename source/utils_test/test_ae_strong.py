...
import source.utils_ae_strong as sua
...
def solve_k(idx):
    i, j = idx
    k_psi_val, k_alpha_val = sua.solve_k_strong(w_alpha, w_psi, w_n[i,j], w_T[i,j], method='iterative', abs_tol=1e-5, rel_tol=1e-5)
    AE_val = sua.AE_local_strong(w_n[i,j], w_T[i,j], w_alpha, w_psi, k_alpha_val, k_psi_val)
