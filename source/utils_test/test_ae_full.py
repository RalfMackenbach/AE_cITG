...
import source.utils_ae_full as sua
import time
...
def solve_k(idx):
    i, j = idx
    k_psi_val, k_alpha_val = sua.solve_k(w_alpha, w_psi, w_n[i,j], w_T[i,j], method='iterative', abs_tol=1e-5, rel_tol=1e-5)
    AE_val = sua.AE_local(w_n[i,j], w_T[i,j], w_alpha, w_psi, k_psi_val, k_alpha_val)
...
if par:
    if __name__ == '__main__':
        ...
        ax[0].set_xlabel(r'$\hat{\omega}_T$')
        ...
        ax[1].set_xlabel(r'$\hat{\omega}_T$')
        ax[1].set_ylabel(r'$\widehat{A}$')
        ...
        ax[2].set_xlabel(r'$\hat{\omega}_T$')
        ax[2].set_ylabel(r'$\widehat{A}$')
